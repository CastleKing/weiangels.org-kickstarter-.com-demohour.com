...
function init(){
	...
	var arrAwardEditing = [];
	$('tbody tr', '#divAwardEditing').each(function (){
		var infoObj = {
			id: $('td', this).eq(4).html(),
			amount: $('td', this).eq(0).html(),
			description: $('td', this).eq(1).html(),
			quantity: $('td', this).eq(2).html(),
			edit:$('td', this).eq(3).html()
		};
		arrAwardEditing.push(infoObj);
	});
	projectInfoObj.award = arrAwardEditing;
	$(':input[name=RewardInfo]').val($.toJSON(projectInfoObj.award));
	
	var arrProjectFAQEditing = [];
	$('tbody tr', '#divProjectFAQEditing').each(function (){
		var infoObj = {
			sequance: $('td', this).eq(0).html(),
			title: $('td', this).eq(1).html(),
			answer: $('td', this).eq(2).html()
		};
		arrProjectFAQEditing.push(infoObj);
	});
	projectInfoObj.FAQ = arrProjectFAQEditing;
	$(':input[name=FAQInfo]').val($.toJSON(projectInfoObj.FAQ));
	
	var arrProjectADEditing = [];
	$('tbody tr', '#divProjectADEditing').each(function (){
		var infoObj = {
			toolPhotoAddr: $('td', this).eq(0).html()
		};
		arrProjectADEditing.push(infoObj);
	});
	projectInfoObj.AD = arrProjectADEditing;
...
function awardInfoAppend(parentTab, awardObj, type){
	...
	$.each(awardObj, function (i, obj){
		...
		arr.push('<td nowrap="nowrap" class="table_hid">' + obj.edit + '</td>');
		arr.push('<td style="display:none;">' + obj.id + '</td>');
		arr.push('</tr>');
	});
...
$(function(){
	...
	$('.link_edit_info').delegate('#Span2', 'click', function (){
		...
		if ($('#ProjectDescDetail_parent', '#divDescInfoEditing').length == 0){
			$(':input[name=ProjectDescDetail]').show();
		}
		if ($('#divProjectDescDetail').html() != ''){
			projectInfoObj.ProjectDescDetail = $('#divProjectDescDetail').html();
			/*var descDetailHtml = $(document.getElementById('ProjectDescDetail_ifr').contentWindow.document.body);
			descDetailHtml.html($('#divProjectDescDetail').html());*/
		}
	});
	...
	*/
	...
	$('#image280x210').bind('click', function (e){
		e.preventDefault();
		$('#dialog_280x210img img').attr('src', $(this).prev().val());
		dialog_280x210.show();
	});
	$('#image200x150').bind('click', function (e){
		e.preventDefault();
			$('#dialog_200x150img img').attr('src', $(this).prev().val());
			dialog_200x150.show();
	});
	$('#image140x105').bind('click', function (e){
		e.preventDefault();
			$('#dialog_140x105img img').attr('src', $(this).prev().val());
			dialog_140x105.show();
	});
	$('#image105x80').bind('click', function (e){
		e.preventDefault();
			$('#dialog_105x80img img').attr('src', $(this).prev().val());
			dialog_105x80.show();
	});
	...
	$('.btnDescInfoEsc').bind('click', function (){
		...
		if (isEdited == '0'){
			...
			$('#divProjectDescDetail, .ProjectDesc, .VideoCode').show();
			$('.hidtitle, #ProjectDescDetail_parent, .upload_avatar, :button, .checkisnull, .error_edit_info', '#divDescInfoEditing').hide();
		}
	});
	...
	$('#editAwardInfo').bind('click', function (e){
		...
		if ($(this).attr('edittype') == 'add'){
			...
			awardInfoAppend(giflist, [{
				...
				quantity: quantity,
				edit: '<a href="#" class="infoUpdate">编辑</a>|<a href="#" class="infoRemove">删除</a>'
			}], 'append');
		} else {
			awardInfoUpdate(
				giflist,
	...
	$('.btnProjectADEsc').bind('click', function (){
		if ($('#hidProjectADEditing').val() == '0'){
			...
			$('.hidtitle, .ADEditingInfo, :input, .error_edit_info', '#divProjectADEditing').hide();
			$('.ADContent').show();
	...
	$('.link_edit_info').delegate('#Span6', 'click', function (){
		...
		$(':input[name=BackEmailContent]').val($('.BackEmailContent').html());
		$(':input[name=CancelEmailContent]').val($('.CancelEmailContent').html());
	});
	...
	$('.btnProjectEmailEsc').bind('click', function (){
		if ($('#hidProjectEmailEditing').val() == '0'){
			...
			$(':input, .hidtitle, .error_edit_info', '#divProjectEmailEditing').hide();
			$('.input_info', '#divProjectEmailEditing').show();
		}
	});
